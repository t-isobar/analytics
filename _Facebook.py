...
class Facebook:
    def __init__(self, token, account, client_name):
        ...
        self.report_dict = {
    "CAMPAIGNS": {
        "fields": {"name": "STRING", "updated_time": "STRING", "created_time": "STRING", "configured_status": "STRING",
                   "effective_status": "STRING", "start_time": "STRING", "stop_time": "STRING", "id": "STRING"}},

    "ADS_STAT": {
        "fields": {"clicks": "INTEGER", "impressions": "INTEGER", "spend": "FLOAT", "video_p100_watched_actions": "INTEGER",
                   "video_p25_watched_actions": "INTEGER", "video_p50_watched_actions": "INTEGER",
                   "video_30_sec_watched_actions": "INTEGER", "video_p75_watched_actions": "INTEGER",
                   "video_p95_watched_actions": "INTEGER", "video_thruplay_watched_actions": "INTEGER",
                   "video_avg_time_watched_actions": "FLOAT", "conversions": "INTEGER", "ad_id": "STRING",
                   "ad_name": "STRING", "campaign_id": "STRING", "campaign_name": "STRING", "adset_id": "STRING",
                   "adset_name": "STRING", "date_start": "STRING", "date_stop": "STRING"}},
    ...
    "ADS_BASIC": {
        "fields": {"adset_id": "STRING", "campaign_id": "STRING", "id": "STRING", "name": "STRING", "status": "STRING",
                   "updated_time": "STRING"}},

    "ADS_LAYOUT": {
        "fields": {"page_id": "STRING", "instagram_actor_id": "STRING", "video_id": "STRING", "message": "STRING",
                   "call_to_action_type": "STRING", "link": "STRING", "link_format": "STRING", "attachment_style": "STRING",
                   "image_hash": "STRING", "link_caption": "STRING", "id": "STRING", "caption": "STRING",
                   "ad_id": "STRING"}},

    "ADSETS": {
        "fields": {"campaign_id": "STRING", "id": "STRING", "name": "STRING", "status": "STRING", "start_time": "STRING",
                   "updated_time": "STRING", "created_time": "STRING", "end_time": "STRING"}}
}

        self.tables_with_schema = {f"{client_name}_Facebook_{report_name}": self.report_dict[report_name]['fields'] for report_name in list(self.report_dict.keys())}
    ...
    def get_ads_or_adsets(self, campaign_ids, method):
        ...
        fields_dict = {"ads-basic": {"fields": "name,id,status,adset_id,updated_time,campaign_id",
                               "limit": 500, "dict_of_keys": {"type": "call_to_action_type"}, "method": "ads"},
                       "ads-layout": {"fields": "adcreatives{object_story_spec}",
                               "limit": 200, "dict_of_keys": {"type": "call_to_action_type"}, "method": "ads"},
                       "adsets": {"fields": "campaign_id,id,name,status,start_time,updated_time,created_time,end_time",
                                  "limit": 500, "dict_of_keys": {}, "method": "adsets"}}

        get_data_params = fields_dict.get(method, False)

        if get_data_params:
            result = self.get_data_with_filtering(campaign_ids_2_slice, get_data_params["method"], get_data_params['fields'], "campaign.id", 'IN', get_data_params['limit'])
            result_data = []

            for one_element_in_list in result:
                if method == 'ads-layout':
                    mid_result_data = self.expand_dict(one_element_in_list['adcreatives'], get_data_params['dict_of_keys'], {})
    ...
    def get_ads_or_campaign_statistics(self, date_from, date_to, level):
        ...
        list_of_days = [datetime.strftime(date_from_dt + timedelta(days=x), "%Y-%m-%d") for x in range(0, (date_to_dt - date_from_dt).days + 1)]
        list_of_days_slice = self.my_slice(list_of_days, 50, [])

        dict_of_params = {"ad": {"fields": "clicks,impressions,spend,video_p100_watched_actions,video_p25_watched_actions,video_p50_watched_actions,video_30_sec_watched_actions,video_p75_watched_actions,video_p95_watched_actions,video_thruplay_watched_actions,video_avg_time_watched_actions,conversions,ad_id,ad_name,campaign_id,campaign_name,adset_id,adset_name",
                                 "limit": 50},
                          "campaign": {"fields": "impressions,clicks,spend,campaign_id,campaign_name,video_p100_watched_actions,video_p25_watched_actions,video_p50_watched_actions,video_30_sec_watched_actions,video_p75_watched_actions,video_p95_watched_actions,video_thruplay_watched_actions,video_avg_time_watched_actions,conversions",
                                       "limit": 100}}
        ...
        if get_data_params:
            ...
            for num, days in enumerate(list_of_days_slice, 1):
                ...
                for day in days:
                    ...
                    batch.append(self.create_betch(url_method, time_range=time_range, fields=get_data_params['fields'], limit=get_data_params['limit'], level=level, time_increment=1))
...
def getFacebookReport(access_token, client_name, account, path_to_bq, date_from, date_to):
    bq = _BigQuery.BigQuery(path_to_bq)
    fb = Facebook(access_token, account, client_name)

    dataset_ID = f"{client_name}_Facebook_{account[4:]}"

    bq.check_or_create_dataset(dataset_ID)
    bq.check_or_create_tables(fb.tables_with_schema, dataset_ID)
    ...
    bq.data_to_insert(ADS_STAT_df, fb.integer_fields, fb.float_fields, fb.string_fields, dataset_ID, f"{client_name}_Facebook_ADS_STAT")
    ...
    bq.data_to_insert(CAMPAIGN_STAT_df, fb.integer_fields, fb.float_fields, fb.string_fields, dataset_ID, f"{client_name}_Facebook_CAMPAIGN_STAT")

    campaigns_ids = CAMPAIGN_STAT_df['campaign_id'].tolist()

    campaigns, header = fb.get_campaigns([])
    campaigns_df = pd.DataFrame(campaigns)
    bq.insert_difference(campaigns_df, fb.integer_fields, fb.float_fields, fb.string_fields, dataset_ID, f"{client_name}_Facebook_CAMPAIGNS", 'id', 'id')

    ads_basic = fb.get_ads_or_adsets(campaigns_ids, "ads-basic")
    ads_basic_df = pd.DataFrame(ads_basic).fillna("<not set>")
    bq.insert_difference(ads_basic_df, fb.integer_fields, fb.float_fields, fb.string_fields, dataset_ID, f"{client_name}_Facebook_ADS_BASIC", 'id', 'id')

    ads_layout = fb.get_ads_or_adsets(campaigns_ids, "ads-layout")
    ads_layout_df = pd.DataFrame(ads_layout).fillna("<not set>")
    bq.insert_difference(ads_layout_df, fb.integer_fields, fb.float_fields, fb.string_fields, dataset_ID, f"{client_name}_Facebook_ADS_LAYOUT", 'id', 'ad_id')

    adsets = fb.get_ads_or_adsets(campaigns_ids, "adsets")
    adsets_df = pd.DataFrame(adsets).fillna("<not set>")
    bq.insert_difference(adsets_df, fb.integer_fields, fb.float_fields, fb.string_fields, dataset_ID, f"{client_name}_Facebook_ADSETS", 'id', 'id')
